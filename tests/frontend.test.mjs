...
test('client palette keeps light blue UI while only Send uses strong blue', async () => {
  const css = await readFile(new URL('../frontend/styles.css', import.meta.url), 'utf8')
  const app = await readFile(new URL('../frontend/app.js', import.meta.url), 'utf8')
  const html = await readFile(new URL('../frontend/template.html', import.meta.url), 'utf8')

  assert.match(css, /--taa-accent:\s*#a6d7eb;/i)
  assert.match(css, /--taa-accent-dark:\s*#7fbfd9;/i)
  assert.match(css, /--taa-bg:\s*#f7fcfe;/i)
  assert.match(css, /--taa-line:\s*#dcecf2;/i)
  assert.match(css, /--taa-user:\s*#a6d7eb;/i)
  assert.match(css, /--taa-button:\s*#1e78ce;/i)
  assert.match(css, /\.taa-reset\s*\{[\s\S]*?border:\s*1px solid var\(--taa-accent\);[\s\S]*?background:\s*#fff;/)
  assert.match(css, /\.taa-send\s*\{[\s\S]*?background:\s*var\(--taa-button\);/)
  assert.match(css, /\.taa-keyboard-toggle\s*\{[\s\S]*?border:\s*1px solid var\(--taa-line\);[\s\S]*?background:\s*#fff;/)
  assert.match(css, /\.taa-message--user \.taa-bubble\s*\{[\s\S]*?background:\s*var\(--taa-user\);[\s\S]*?color:\s*#111;/)
  assert.match(css, /\.taa-typing-dot\s*\{[\s\S]*?background:\s*var\(--taa-accent\);/)
  assert.match(app, /config\.accent\s*\|\|\s*['"]#A6D7EB['"]/)
  assert.match(html, /accent:\s*['"]#A6D7EB['"]/)
  assert.match(html, /<span>Select dialect<\/span>/)
})
